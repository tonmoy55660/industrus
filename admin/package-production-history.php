<?php
$title = "Industrus | Production history";
...
if (isset($_GET['order-id'])) {
    $sql = "SELECT * FROM production_track WHERE department_id = 4 AND order_id = ?  ";
...
$sql = "SELECT * FROM production_track WHERE department_id = 4 AND production_date = ?  ";
$stmt = $conn->prepare($sql);
...
$sql = "SELECT SUM(production_amount) as total FROM `production_track` WHERE order_id=? AND department_id = 4";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $orderId);
$orderId = mysqli_real_escape_string($conn, $_GET['order-id']);
$stmt->execute();
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td><a href="package-production-history?order-id=<?= $_GET['order-id'] ?>&&date=<?= $value['production_date'] ?>&&amount=<?= $value['production_amount'] ?>" class="btn btn-info btn-sm"><i class="fas fa-edit"></i>Edit</a></td>
                            </tr>
