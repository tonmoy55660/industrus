...
$conn = connect();
$sql = "SELECT * FROM order_tasks WHERE department_id = 4";
...
$sql = "SELECT SUM(CASE WHEN Status = 2 THEN 1 ELSE 0 END) AS `Finished`, SUM(CASE WHEN Status = 1 THEN 1 ELSE 0 END) AS `In-progress` FROM order_tasks WHERE department_id = 4";
$stmt = $conn->prepare($sql);
$stmt->execute();
$result = $stmt->get_result();
...
<script>
    ...
    function generateDayPDF() {
        ...
        doc.save('Package order days comparison.pdf');
    }
    ...
    function generateStatusPDF() {
        ...
        doc.save('Package order status.pdf');
    }
